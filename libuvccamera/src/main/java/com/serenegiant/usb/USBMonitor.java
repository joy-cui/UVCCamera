...
//			if (DEBUG) Log.i(TAG, "unregister:");
		...
		if (deviceList != null) {
			for (final UsbDevice device: deviceList.values() ) {
				if ((filter == null) || (filter.matches(device) && !filter.isExclude)) {
					Log.e("","deviceList...."+device.getDeviceId()+" : "+device.getDeviceName());
					result.add(device);
				}
			}
		}
	...
	public synchronized boolean requestPermission(final UsbDevice device) {
	 Log.e(TAG, "requestPermission:device=" + device);
		boolean result = false;
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					Log.e(TAG, "requestPermission:processConnect...22.=" + device);
					...
					try {
						...
						Log.e(TAG, "requestPermission:requestPermission..33..=" + device);
					} catch (final Exception e) {
				...
				Log.e(TAG, "requestPermission:processCancel..44..=" + device);
				processCancel(device);
				result = true;
			}
		} else {
